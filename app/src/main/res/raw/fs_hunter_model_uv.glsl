...
void main() {
    ...
    float distance = length(lightPosition - v_Position_Local);
...
//    vec3 lightVectorGlobal = normalize(uLightPos - v_Position_Global);
    vec3 lightVectorLocal = normalize(lightPosition - v_Position_Local);
...
//    diffuseLocal = diffuseLocal * (1.0 / (1.0 + (0.03 * distance)));
//    diffuseLocal = diffuseLocal * 1.0;
